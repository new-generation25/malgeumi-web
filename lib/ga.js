...
/**
 * GA4는 한 속성당 동시 요청을 10개까지만 받는다. 화면을 연달아 새로고침하면
 * 앞 요청이 끝나기 전에 다음 것이 겹쳐 그 선을 넘는다. 그때는 잠깐 쉬었다
 * 다시 부르면 통과한다. 구글도 이 방식을 권한다.
 */
async function callGa(propertyId, method, body, attempt = 0) {
  ...
  if (!res.ok) {
    ...
    if (/concurrent requests|RESOURCE_EXHAUSTED/i.test(msg) && attempt < 5) {
      await sleep(500 * Math.pow(2, attempt)); // 0.5 → 1 → 2 → 4 → 8초
      return callGa(propertyId, method, body, attempt + 1);
    }
